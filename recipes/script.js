const recipes = [
  ...
  },
  {
    ...
    tags: ["dessert"],
    description:
      "Delicious soft chocolate chip cookies with coconut.",
  ...
  },
  {
    author: "Ester Kocht",
    url:
      "https://www.esterkocht.com/german-gooseberry-cake-with-vanilla-cream-and-crumble/",
    isBasedOn: "",
    cookTime: "45min",
...
function filterRecipes(query) {
  ...
  return filteredRecipes.sort(compareRecipes);
}

function searchHandler(event) {
  event.preventDefault();

  const matchingRecipes = filterRecipes(searchInput.value);

  renderRecipes(matchingRecipes);
}

function init() {
  const randomRecipeIndex = random(recipes.length);
